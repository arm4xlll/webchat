      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          <View style={styles.logoWrap}>
            ...
            <Text style={[styles.appSub, { color: t.muted }]}>Общайтесь быстро и безопасно</Text>
          </View>

          <View style={[styles.card, { backgroundColor: t.card, borderColor: t.border }]}>
            <Text style={[styles.title, { color: t.text }]}>Вход</Text>
            <Text style={[styles.subtitle, { color: t.muted }]}>Введите данные своего аккаунта</Text>
            ...
            <View style={styles.field}>
              ...
              <View style={[styles.inputRow, { backgroundColor: t.inputBg, borderColor: t.border }]}>
                ...
                <TextInput style={[styles.input, { color: t.text }]} value={password} onChangeText={setPassword}
                  placeholder="••••••••" placeholderTextColor={t.muted} secureTextEntry={!showPass}
            ...
            <TouchableOpacity style={[styles.button, { backgroundColor: loading ? t.muted : t.primary }]}
              ...
              {loading ? <ActivityIndicator color="#fff" size="small" /> : <Text style={styles.buttonText}>Войти</Text>}
            </TouchableOpacity>
          </View>

          <View style={styles.footer}>
            <Text style={[styles.footerText, { color: t.muted }]}>Нет аккаунта? </Text>
            <Pressable onPress={() => navigation.navigate('Register')}>
              <Text style={[styles.link, { color: t.primary }]}>Зарегистрироваться</Text>
            </Pressable>
          </View>
        </ScrollView>
